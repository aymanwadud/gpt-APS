...
# from utils.database import Appointment, Session Removed this
import pandas as pd
    ...
    def load_appointments():
        ...
        if uploaded_file is not None:
            try:
                ...
                clear_database()
                appointments = ingestion_agent.ingest_data('temp.pdf')
                st.success("Appointments loaded successfully!")
                st.session_state.queue = appointments
                st.rerun()
            ...
            try:
              ...
              for row in rowsToUpdate:
                for appt in st.session_state.queue:
                  if appt['id'] == row['id']:
                      appt['sl'] = row['sl']
                      new_queue.append(appt)
              st.session_state.queue = sorted(new_queue, key=lambda x: x['sl'])
              st.session_state["agGrid_key"] = None
              st.rerun()
            except:
               pass
